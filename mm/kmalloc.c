...
//
// @description:
//      This is a copy of malloc.c but calls the kernel
...
//        DONE
//      - Finish the kmalloc and work on kfree.
//        DONE
...
void kmalloc_unit_test()
{
    ...
    printk("expect line 92\n");
    {
        ...
        {
            ptr1 = (char *)kmalloc(10);
            strcpy(ptr1,"world");
            printk("%s\n", ptr1);
        }
        printk("expect line 146\n");
        {
            ptr2 = (char *)kmalloc(20);
    ...
    table = (char **)kmalloc(128 * sizeof(char **));
    for(i = 0; i < 128; ++i) {
        table[i] = (char *)kmalloc(2 * (i+1) * 100);
        sprintk(table[i],"%d",i);
        if(i == 95 || i == 127) {
            printk("table[i]=%s i=%d table pointer=%d\n",table[i],i,&table[i]);
        }
    }     
    for(i = 0; i < 128; ++i) {
        sprintk(buf,"%d",i);
        if(strcmp(table[i],buf)) {
            printk("allocation error slot [%d] table[i]=%s buf=%s table pointer=%d\n",
                    i,table[i],buf,&table[i]);
            break;
        }
    }
    if(i == 128) {
